...
function log_vault_command() {
  ...
  echo "### Using the Vault CLI" >> $DEMO_RESULT
  echo "" >> $DEMO_RESULT
  code >> $DEMO_RESULT
  printf "%s\n\n" "$@" >> $DEMO_RESULT
  code >> $DEMO_RESULT
...
function log() {
  ...
  echo "### Generated Output" >> $DEMO_RESULT
  printf "\n%s\n" "${date}" >> $DEMO_RESULT
  json >> $DEMO_RESULT
  echo "$@" | jq . >> $DEMO_RESULT
  code >> $DEMO_RESULT
